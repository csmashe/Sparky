...
void ReadSparkResponse() {
  delay(100); while (SerialBT.available()) {
    incomingBLEByte = SerialBT.read(); // Serial.print(incomingBLEByte,HEX);Serial.print(",");
  }
...
void DriveStatusToggle() {
  ...
  if (FXDriveStatus[0] == FX_ON) {
    FXDriveStatus[0] = FX_OFF;
    Serial.println("=OFF");
  }
  else {
    FXDriveStatus[0] = FX_ON;
    Serial.println("=ON");
  }


  if (FXDriveType == "TubeDrive") {
    SerialBT.write(TubeDriveCodes, sizeof(TubeDriveCodes));
    SerialBT.write(FXDriveStatus, 1);
    SerialBT.write(EndOfMessage, 1);
  }
  else if (FXDriveType == "Booster") {
    SerialBT.write(BoosterCodes, sizeof(BoosterCodes));
    SerialBT.write(FXDriveStatus, 1);
    SerialBT.write(EndOfMessage, 1);
  }
  else if (FXDriveType == "Fuzz") {
    SerialBT.write(FuzzCodes, sizeof(FuzzCodes));
    SerialBT.write(FXDriveStatus, 1);
    SerialBT.write(EndOfMessage, 1);
  }
  else if (FXDriveType == "OverDrive") {
    SerialBT.write(OverDriveCodes, sizeof(OverDriveCodes));
    SerialBT.write(FXDriveStatus, 1);
    SerialBT.write(EndOfMessage, 1);
  }
...
void ModStatusToggle() {
  ...
  if (FXModStatus[0] == FX_ON) {
    FXModStatus[0] = FX_OFF;
    Serial.println("=OFF");
  }
  else {
    FXModStatus[0] = FX_ON;
  ...
  if (FXModType == "Vibe") {
    SerialBT.write(VibeSwitchCodes, sizeof(VibeSwitchCodes));
    SerialBT.write(FXModStatus, 1);
    SerialBT.write(EndOfMessage, 1);
  }
  else if (FXModType == "DigitalChorus") {
    SerialBT.write(DigitalChorusSwitchCodes, sizeof(DigitalChorusSwitchCodes));
    SerialBT.write(FXModStatus, 1);
    SerialBT.write(EndOfMessage, 1);
  }
  else if (FXModType == "Tremolo") {
    SerialBT.write(TremoloSwitchCodes, sizeof(TremoloSwitchCodes));
    SerialBT.write(FXModStatus, 1);
    SerialBT.write(EndOfMessage, 1);
  }
  else if (FXModType == "Tremolator") {
    SerialBT.write(TremolatorSwitchCodes, sizeof(TremolatorSwitchCodes));
    SerialBT.write(FXModStatus, 1);
    SerialBT.write(EndOfMessage, 1);
  }
  else if (FXModType == "Phaser") {
    SerialBT.write(PhaserSwitchCodes, sizeof(PhaserSwitchCodes));
    SerialBT.write(FXModStatus, 1);
    SerialBT.write(EndOfMessage, 1);
  }
  else if (FXModType == "Cloner") {
    SerialBT.write(ClonerSwitchCodes, sizeof(ClonerSwitchCodes));
    SerialBT.write(FXModStatus, 1);
    SerialBT.write(EndOfMessage, 1);
  }
  else if (FXModType == "Flanger") {
    SerialBT.write(FlangerSwitchCodes, sizeof(FlangerSwitchCodes));
    SerialBT.write(FXModStatus, 1);
    SerialBT.write(EndOfMessage, 1);
  }
...
void DelayStatusToggle() {
  ...
  if (FXDelayStatus[0] == FX_ON) {
    FXDelayStatus[0] = FX_OFF;
    Serial.println("=OFF");
  }
  else {
    FXDelayStatus[0] = FX_ON;
    Serial.println("=ON");
  }


  if (FXDelayType == "DelayEcho") {
    SerialBT.write(DelayEchoCodes, sizeof(DelayEchoCodes));
    SerialBT.write(FXDelayStatus, 1);
    SerialBT.write(EndOfMessage, 1);
  }
  else if (FXDelayType == "DigitalDelay") {
    SerialBT.write(DigitalDelayCodes, sizeof(DigitalDelayCodes));
    SerialBT.write(FXDelayStatus, 1);
    SerialBT.write(EndOfMessage, 1);
  }
  else if (FXDelayType == "EchoTape") {
    SerialBT.write(EchoTapeCodes, sizeof(EchoTapeCodes));
    SerialBT.write(FXDelayStatus, 1);
    SerialBT.write(EndOfMessage, 1);
  }
  else if (FXDelayType == "VintageDelay") {
    SerialBT.write(VintageDelayCodes, sizeof(VintageDelayCodes));
    SerialBT.write(FXDelayStatus, 1);
    SerialBT.write(EndOfMessage, 1);
  }
  else if (FXDelayType == "MultiHeadDelay") {
    SerialBT.write(MultiHeadDelayCodes, sizeof(MultiHeadDelayCodes));
    SerialBT.write(FXDelayStatus, 1);
    SerialBT.write(EndOfMessage, 1);
  }
...
void ReverbStatusToggle() {
  ...
  if (FXReverbStatus[0] == FX_ON) {
    FXReverbStatus[0] = FX_OFF;
    Serial.println("=OFF");
  }
  else {
    FXReverbStatus[0] = FX_ON;
    Serial.println("=ON");
  }
